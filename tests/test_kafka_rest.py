...
JMX_CHECK = """bash -c "\
    echo 'get -b kafka.rest:type=jetty-metrics,name=connections-active' |
...
class JMXTest(unittest.TestCase):
    ...
    def test_jmx_host_network(self):
        ...
        logs = utils.run_docker_command(
            ...
            command=JMX_CHECK.format(jmx_hostname="localhost", jmx_port="8082"),
            host_config={'NetworkMode': 'host'})
    ...
    #     logs = utils.run_docker_command(
    #         image="confluentinc/cp-jmxterm",
    #         command=JMX_CHECK.format(jmx_hostname="kafka-rest-bridged-jmx", jmx_port="8082"),
    #         host_config={'NetworkMode': 'standalone-network-test_zk'})
    #     self.assertTrue("Version = 0.10.0.0-cp1;" in logs)
