...
Start Kafka.
  ...
  .. note::
    ...
    You'll notice that the ``KAFKA_ADVERTISED_LISTENERS`` variable is set to ``kafka:9092``.  This will make Kafka accessible to other containers by advertising it's location on the Docker network.  You also passed in the ZooKeeper port that you used when launching that container a moment ago.
