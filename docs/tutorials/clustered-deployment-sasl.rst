...
4. Build and run the kerberos image
  ...
  .. sourcecode:: bash
    ...
    docker build -t confluentinc/cp-kerberos:3.3.0 .
    ...
    docker run -d \
      ...
      confluentinc/cp-kerberos:3.3.0
...
6. Run a 3-node |zk| ensemble with SASL enabled.
  ...
  .. sourcecode:: bash
     ...
     for i in 22181 32181 42181; do
        docker run --net=host --rm confluentinc/cp-zookeeper:3.3.0 bash -c "echo stat | nc quickstart.confluent.io $i | grep Mode"
     done
